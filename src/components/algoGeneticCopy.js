export function scheduleBreaksGeneticTest(employees, setMaximumIntersection) {
  ...
  function countBreaksAtTime(schedules, time) {
    let breaksAtTime = 0;
    for (const employee in schedules) {
      for (const breakType in schedules[employee]) {
        if (
          timeToMinutes(schedules[employee][breakType].start) ===
          timeToMinutes(time)
        ) {
          breaksAtTime++;
        }
  ...
  function createBreakObject(breakStart, breakEnd) {
    return {
  ...
  function generateSchedule(shiftStart, shiftEnd, breaksScheduleTemp) {
    ...
    // Check if there are already two breaks scheduled between 08:45 and 09:00
    let breaksBetween0845and0900 = countBreaksAtTime(
      breaksScheduleTemp,
      "08:45"
    );

    // If there are already two breaks scheduled between 08:45 and 09:00, schedule the break after 09:00
    if (breaksBetween0845and0900 >= 2) {
      breakStart = timeToMinutes("09:00");
      breakEnd = breakStart + 15;
    }

    // Make sure the first break starts no earlier than shiftStart + 60
    if (breakStart < shiftStart + 60) {
      breakStart = shiftStart + 60;
      breakEnd = breakStart + 15;
    }

    // Check if there are already three breaks scheduled at 09:00
    let breaksAt0900 = countBreaksAtTime(breaksScheduleTemp, "09:00");

    // If there are already three breaks scheduled at 09:00, schedule the break after the last scheduled break at 09:00
    if (breaksAt0900 >= 3) {
      let latestBreakAt0900 = timeToMinutes("09:00");
      for (const employee in breaksScheduleTemp) {
        if (
          timeToMinutes(breaksScheduleTemp[employee].firstBreak.start) === timeToMinutes("09:00")
        ) {
          latestBreakAt0900 = Math.max(
            latestBreakAt0900,
            timeToMinutes(breaksScheduleTemp[employee].firstBreak.end)
          );
        }
      }
      breakStart = latestBreakAt0900;
      breakStart = Math.max( latestBreakAt0900, shiftStart + 60);
      breakEnd = breakStart + 15;
    }

    schedule.firstBreak = createBreakObject(breakStart, breakEnd);

    //secondBreak
    breakStart = breakEnd + 60;
    breakEnd = breakStart + 15;
    schedule.secondBreak = createBreakObject(breakStart, breakEnd);

    //lunch
    breakStart = timeToMinutes("12:00");
    breakEnd = breakStart + 45;
    schedule.lunch = createBreakObject(breakStart, breakEnd);

    //thirdBreak
    breakStart = Math.max(breakEnd + 60, timeToMinutes("14:00"));

    // Make sure the third break starts no later than shiftEnd - 75
    if (breakStart > shiftEnd - 75) {
      breakStart = shiftEnd - 75;
    }

    breakEnd = breakStart + 15;

    schedule.thirdBreak = createBreakObject(breakStart, breakEnd);
